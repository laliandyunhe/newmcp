...
(function () {
  ...
  function emptyServer() {
    return {
      ...
      cachedTools: [], // 上次测试连接拿到的工具列表，会持久化，重新打开插件也还在
    };
  }
        ...
        async mount(container, roche) {
          ...
          async function testAndListTools() {
            ...
            try {
              ...
              state.editing.cachedTools = (list.tools || []).map((t) => ({
                name: t.name,
              ...
              state.testStatus = "success";
              state.testMessage = `连接成功，延迟 ${latency}ms，发现 ${state.editing.cachedTools.length} 个工具`;
            } catch (e) {
              state.testStatus = "error";
          ...
          function saveEditing() {
            ...
            const cleaned = { ...state.editing }; // cachedTools 保留，下次打开不用重新测试
          ...
          function renderList() {
            ...
            title.className = "rpmd-title";
            title.textContent = "MCP 调试面板";
          ...
          function renderBasicTab() {
            ...
            testBtn.textContent =
              state.editing.cachedTools && state.editing.cachedTools.length > 0
                ? "重新测试连接"
                : "测试连接并拉取工具列表";
